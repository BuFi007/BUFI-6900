...
5. **Exit is a governance action.** Shares sit on the MSCA; `redeem`/`withdraw` go through `execute`, which the AddressBook gates (the vault must be an allowlisted recipient for the *redeem* leg — add it at ceremony). A V2 illiquid exit (force deallocation, in-kind) is a separate multisig decision; never automate it.

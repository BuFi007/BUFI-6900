...
- **Earn** (`BufiEarnModule`): `computeEarnConfigHash` (byte-matches the on-chain `setConfig` hash), `encodeAutoEarn`, `encodeChangeConfigHash`, `encodeEarnModuleInstallData`, `encodeInstallEarnModule`, `getEarnConfigs`.

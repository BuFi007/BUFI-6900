...
//
// BUFI provenance: ported from alchemyplatform/modular-account v1.0.1 `src/plugins/session/ISessionKeyPlugin.sol`,
// commit 1ceb7935, for Circle's UpgradableMSCA (ERC-6900 v0.7 / ERC-4337 v0.7). The interface is renamed
// `IBufiSessionKeyPlugin`; every function, event, error, struct and enum is byte-for-byte the upstream ABI, so
// `type(IBufiSessionKeyPlugin).interfaceId == type(ISessionKeyPlugin).interfaceId`. `Call` now comes from Circle's
// `Structs.sol` (identical layout). The unused `UserOperation` import is dropped. See PORT-NOTES.md.

pragma solidity 0.8.24;

import {Call} from "@circle/msca/6900/v0.7/common/Structs.sol";

interface IBufiSessionKeyPlugin {
    enum FunctionId {
    ...
    function getAccessControlType(address account, address sessionKey) external view returns (ContractAccessControlType);
    ...
    /// access control type).
    function isSelectorOnAccessControlList(address account, address sessionKey, address targetAddress, bytes4 selector)
        external
        view
        returns (bool isOnList);

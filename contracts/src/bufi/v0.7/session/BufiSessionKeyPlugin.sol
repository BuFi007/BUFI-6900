...
//
// BUFI provenance: ported from alchemyplatform/modular-account v1.0.1 `src/plugins/session/SessionKeyPlugin.sol`,
// commit 1ceb7935, for Circle's production `UpgradableMSCA` (ERC-6900 v0.7 / ERC-4337 v0.7 `PackedUserOperation`).
// Every deviation from the audited original is marked `PORT:` inline and listed in PORT-NOTES.md.
...
// PORT: ERC-4337 v0.7 struct, Circle's ERC-6900 v0.7 plugin base / interfaces / manifest structs, and the
// modular-account-libs AssociatedLinkedListSet (byte-identical to MAv1's; Circle's own plugins use the same lib).
import {PackedUserOperation} from "@account-abstraction/contracts/interfaces/PackedUserOperation.sol";
import {SIG_VALIDATION_FAILED, SIG_VALIDATION_SUCCEEDED as SIG_VALIDATION_PASSED} from "@circle/common/Constants.sol";
import {CastLib} from "@circle/libs/CastLib.sol";
...
import {SENTINEL_VALUE, SetValue} from "@modular-account-libs/libraries/Constants.sol";

import {IBufiSessionKeyPlugin} from "./IBufiSessionKeyPlugin.sol";
import {SessionKeyPermissions} from "./permissions/SessionKeyPermissions.sol";

/// @title BUFI Session Key Plugin
/// @author Alchemy (original), BUFI (port to Circle UpgradableMSCA / ERC-4337 v0.7)
...
///
/// @dev BUFI uses it as the "agentic wallet policy" primitive: an AI agent holds a session key EOA and the
/// account's owners (Circle's WeightedWebauthnMultisigPlugin) grant it {scope, budget, expiry}. On a Circle
/// weighted-multisig account the installer passes
/// `dependencies = [FunctionReference(weightedPlugin, 1), FunctionReference(weightedPlugin, 0)]`: slot 0 (runtime
/// validation of the key-management functions) points at a function id the weighted plugin deliberately does NOT
/// implement, so owners can only manage keys through userOps; slot 1 is `USER_OP_VALIDATION_OWNER`.
contract BufiSessionKeyPlugin is IBufiSessionKeyPlugin, SessionKeyPermissions, BasePlugin {
    // PORT: OpenZeppelin 5 splits `toEthSignedMessageHash` out of ECDSA into MessageHashUtils.
    using MessageHashUtils for bytes32;
    using ECDSA for bytes32;
    using AssociatedLinkedListSetLib for AssociatedLinkedListSet;

    string internal constant _NAME = "BUFI Session Key Plugin";
    string internal constant _VERSION = "1.0.1";
    string internal constant _AUTHOR = "BUFI (ported from Alchemy Modular Account v1.0.1)";
    ...
    /// @inheritdoc IBufiSessionKeyPlugin
    ...
    /// @inheritdoc IBufiSessionKeyPlugin
    function rotateSessionKey(address oldSessionKey, bytes32 predecessor, address newSessionKey) external override {
    ...
    /// @dev PORT: Circle's BasePlugin has no `_onInstall` indirection, so the audited `_onInstall` body lives here
    /// under `onInstall` directly. MAv1's BasePlugin additionally rejected callers with no code
    /// (`NotContractCaller`). That check is dropped for parity with Circle's BasePlugin and production plugins,
    /// none of which carry it: Circle's PluginManager runs under delegatecall, so `onInstall` is always called
    /// from the account's own context, and Circle's factory installs plugins from inside the ERC1967 proxy
    /// constructor (`initializeUpgradableMSCA` is the proxy's constructor calldata), where the account has no
    /// code yet. A caller with no code can only initialise state keyed by its own address. The
    /// `isNotInitialized` guard is Circle's, with identical semantics.
    ...
    /// @inheritdoc BasePlugin
    /// @dev PORT: takes ERC-4337 v0.7 `PackedUserOperation`; the signature, membership and permission logic is the
    /// audited original. OpenZeppelin 5's `tryRecover` returns a third (error-argument) value that is ignored.
    ...
    /// @inheritdoc BasePlugin
    /// @dev Layout is the audited original's, expressed with Circle's (ERC-6900 v0.7 spec-identical) manifest
    /// structs. `dependencyInterfaceIds` reference Circle's `IPlugin`, so the manifest hash differs from Alchemy's.
    function pluginManifest() external pure override returns (PluginManifest memory) {
        ...
        manifest.userOpValidationFunctions[1] = ManifestAssociatedFunction({
            executionSelector: this.addSessionKey.selector, associatedFunction: ownerUserOpValidationFunction
        });
        manifest.userOpValidationFunctions[2] = ManifestAssociatedFunction({
            executionSelector: this.removeSessionKey.selector, associatedFunction: ownerUserOpValidationFunction
        });
        manifest.userOpValidationFunctions[3] = ManifestAssociatedFunction({
            executionSelector: this.rotateSessionKey.selector, associatedFunction: ownerUserOpValidationFunction
        });
        manifest.userOpValidationFunctions[4] = ManifestAssociatedFunction({
            executionSelector: this.updateKeyPermissions.selector, associatedFunction: ownerUserOpValidationFunction
        });
        ...
        // PORT note: Circle's BaseMSCA rejects the runtime path one step earlier than MAv1 — an execution function
        // with no runtime validation function reverts `InvalidValidationFunctionId` before any pre-runtime hook
        // runs — so the always-deny hook below is a second, redundant lock. It is kept for manifest fidelity.
        ...
        manifest.runtimeValidationFunctions[2] = ManifestAssociatedFunction({
            executionSelector: this.rotateSessionKey.selector, associatedFunction: ownerRuntimeValidationFunction
        });
        manifest.runtimeValidationFunctions[3] = ManifestAssociatedFunction({
            executionSelector: this.updateKeyPermissions.selector, associatedFunction: ownerRuntimeValidationFunction
        });
    ...
    function pluginMetadata() external pure virtual override returns (PluginMetadata memory) {
        ...
        metadata.permissionDescriptors[0] = SelectorPermission({
            functionSelector: this.addSessionKey.selector, permissionDescription: modifySessionKeys
        });
        metadata.permissionDescriptors[1] = SelectorPermission({
            functionSelector: this.removeSessionKey.selector, permissionDescription: modifySessionKeys
        });
        metadata.permissionDescriptors[2] = SelectorPermission({
            functionSelector: this.rotateSessionKey.selector, permissionDescription: modifySessionKeys
        });
        metadata.permissionDescriptors[3] = SelectorPermission({
            functionSelector: this.updateKeyPermissions.selector, permissionDescription: modifySessionKeyPermissions
        });
    ...
    function supportsInterface(bytes4 interfaceId) public view override returns (bool) {
        return interfaceId == type(IBufiSessionKeyPlugin).interfaceId || super.supportsInterface(interfaceId);
    }

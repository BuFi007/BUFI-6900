...
//
// BUFI provenance: ported from alchemyplatform/modular-account v1.0.1
// `src/plugins/session/permissions/SessionKeyPermissions.sol`, commit 1ceb7935, for Circle's UpgradableMSCA
// (ERC-6900 v0.7 / ERC-4337 v0.7). Every deviation from the audited original is marked `PORT:` inline and listed
// in ../PORT-NOTES.md. The only behavioural change is the ERC-4337 v0.7 "max gas cost" formula in
// `_checkUserOpPermissions` (see `_getMaxGasCost`).

pragma solidity 0.8.24;

import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";

// PORT: ERC-4337 v0.7 `PackedUserOperation` + the EntryPoint's own unpacking library replace the v0.6
// `UserOperation` struct and its flat gas fields.
import {UserOperationLib} from "@account-abstraction/contracts/core/UserOperationLib.sol";
import {PackedUserOperation} from "@account-abstraction/contracts/interfaces/PackedUserOperation.sol";
// PORT: Circle's `Call` (identical layout) and Circle's validation constants (identical values 0 / 1). The Circle
// constant is imported under Alchemy's name so the audited function bodies below are unchanged.
...
abstract contract SessionKeyPermissions is IBufiSessionKeyPlugin, SessionKeyPermissionsLoupe {
        ...
        returns (uint256)
    {
        ...
        if (!sessionKeyData.nativeTokenSpendLimitBypassed) {
            (bool spendLimitSuccess, uint48 spendLimitValidAfter) = _checkSpendLimitUsage(
                nativeTokenSpend, sessionKeyData.nativeTokenSpendLimitTimeInfo, sessionKeyData.nativeTokenSpendLimit
            );
        ...
        if (sessionKeyData.hasGasLimit) {
            ...
            // `paymasterAndData`. `_getMaxGasCost` reproduces `EntryPoint._getRequiredPrefund` (v0.7) exactly.
            uint256 maxGasFee = _getMaxGasCost(userOp);
            (bool gasLimitSuccess, uint48 gasLimitValidAfter) = _checkAndUpdateGasLimitUsage(maxGasFee, sessionKeyData);
        ...
        if (sessionKeyData.hasRequiredPaymaster) {
            ...
            // PORT: EntryPoint v0.7 enforces a stricter minimum — a non-empty `paymasterAndData` must be at least
            // 52 bytes (20-byte paymaster ‖ two 16-byte gas limits) — so the first-20-bytes read below is still
            // exactly the paymaster the EntryPoint will charge.
        ...
        return uint160(validationSuccess ? SIG_VALIDATION_PASSED : SIG_VALIDATION_FAILED) | (uint256(validUntil) << 160)
            | (uint256(currentValidAfter) << 208);
    }

    /// @dev PORT (new helper): the maximum native-token amount EntryPoint v0.7 can charge the account for `userOp`,
    /// computed exactly like `EntryPoint._getRequiredPrefund`:
    ///   (verificationGasLimit + callGasLimit + paymasterVerificationGasLimit + paymasterPostOpGasLimit
    ///    + preVerificationGas) * maxFeePerGas
    /// Field unpacking uses the EntryPoint's own `UserOperationLib`, never hand-rolled shifts. When there is no
    /// paymaster both paymaster gas limits are zero, as in the EntryPoint. The EntryPoint rejects any non-empty
    /// `paymasterAndData` shorter than 52 bytes ("AA93 invalid paymasterAndData") before account validation runs;
    /// if this function is ever reached with such data, the calldata slice inside `unpackPaymasterStaticFields`
    /// reverts, which is fail-closed. Overflow is impossible for EntryPoint-validated ops (every gas value is
    /// capped at uint120 by "AA94 gas values overflow"); the checked arithmetic reverts otherwise.
    function _getMaxGasCost(PackedUserOperation calldata userOp) internal pure returns (uint256) {
        ...
        uint256 requiredGas = userOp.unpackVerificationGasLimit() + userOp.unpackCallGasLimit()
            + paymasterVerificationGasLimit + paymasterPostOpGasLimit + userOp.preVerificationGas;
        return requiredGas * userOp.unpackMaxFeePerGas();
    }
    ...
    function _performSessionKeyPermissionsUpdate(
        ...
        if (updateSelector == ISessionKeyPermissionsUpdates.setAccessListType.selector) {
            ContractAccessControlType contractAccessControlType = abi.decode(update[4:], (ContractAccessControlType));
            ...
            (address contractAddress, bytes4 selector, bool isOnList) = abi.decode(update[4:], (address, bytes4, bool));
    ...
    function _updateAccessListFunctionEntry(SessionKeyId keyId, address contractAddress, bytes4 selector, bool isOnList)
        internal
    {
    ...
    function _updateTimeRange(SessionKeyData storage sessionKeyData, uint48 validAfter, uint48 validUntil) internal {
        ...
        internal
    {
        ...
        tokenContractData.isERC20WithSpendLimit = _updateSpendLimits(
            spendLimit, refreshInterval, tokenContractData.erc20SpendLimitTimeInfo, tokenContractData.erc20SpendLimit
        );
    }
        ...
        internal
    {
        ...
        sessionKeyData.hasGasLimit =
            _updateSpendLimits(spendLimit, refreshInterval, sessionKeyData.gasLimitTimeInfo, sessionKeyData.gasLimit);
    }

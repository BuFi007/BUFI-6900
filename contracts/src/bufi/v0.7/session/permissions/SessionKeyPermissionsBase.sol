...
//
// BUFI provenance: ported VERBATIM from alchemyplatform/modular-account v1.0.1
// `src/plugins/session/permissions/SessionKeyPermissionsBase.sol`, commit 1ceb7935. Only the pragma and the two
// import paths change (`PluginStorageLib` is vendored under ../libraries/, the interface is renamed). The storage
// key derivation — prefixes, batch index, key widths — is untouched. See ../PORT-NOTES.md.
...
abstract contract SessionKeyPermissionsBase is IBufiSessionKeyPlugin {
    ...
    function _loadSessionKeyId(address associated, address sessionKey) internal view returns (SessionKeyId keyId) {

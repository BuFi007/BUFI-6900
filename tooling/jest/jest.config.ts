...
/**
 * Walk up from a package directory to the workspace root.
 *
 * BUFI modification: the upstream template only recognised a pnpm workspace
 * (`pnpm-workspace.yaml`). This fork runs under bun workspaces, whose root is
 * marked by `bun.lock`, so both markers are accepted.
 */
